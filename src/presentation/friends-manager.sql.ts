...
import { FriendsController } from '../controllers/friends.controller.sql.js';
import { displayTable } from '../core/display-table.js';
import { closePool } from '../core/storage/db.sql.js';
...
const addFriend = async () => {
  ...
  while (true) {
    ...
    try {
      ...
      if (updatedFriend) displayTable([updatedFriend]);
      return;
    } catch (err: unknown) {
...
const updateFriend = async () => {
  ...
  if (!targetQuery.trim()) {
    ...
    const searchResult = await controller.searchFriends(targetQuery);
...
const removeFriend = async () => {
  ...
  if (friendToRemove) displayTable([friendToRemove]);
